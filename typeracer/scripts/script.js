...
submitPopUp = document.querySelector("#submit"),
...
wpmResult = document.querySelector("#submit__wpmResult"),
ksResult = document.querySelector("#submit__keystrokesResult"),
vWords = document.querySelector("#submit__validWords"),
uvWords = document.querySelector("#submit__unvalidWords");
...
function getWords(event) {
    ...
    if(key === 32 && ifclicked) {
        ...
        let color = generatedWords[counter-1] === v ? "green" : "red";
        document.querySelector(`.span${counter-1}`).style.color = color;
        wordsInput.value = "";
    }
}
